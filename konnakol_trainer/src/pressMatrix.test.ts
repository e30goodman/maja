...
function applyPatchToState(state: PressState, patch: ReturnType<typeof tilePress>): PressState {
	return {
		...
		cellStepMasks: patch.cellStepMasks ?? state.cellStepMasks,
...
function testRoundtripMono() {
	...
	state.cellStepMasks['0-0'] = [true, false];
	...
	assert.deepEqual(frame.stepMasks[0], [true, false]);
	...
	assert.deepEqual(patch.cellStepMasks!['5-0'], [true, false]);
...
function testTileAllLayers() {
	...
	state.cellStepMasks['0-0'] = [true, false, true];
	...
	for (const r of [1, 2]) {
		...
		assert.deepEqual(patch.cellStepMasks![`${r}-0`], [true, false, true]);
...
function testDropAllLayers() {
	...
	for (let r = 0; r < 8; r++) {
		...
		state.cellStepMasks[`${r}-0`] = [true, false];
	...
	for (const r of [3, 4, 5, 6, 7]) {
		...
		assert.equal(patch.cellStepMasks![`${r}-0`], undefined);

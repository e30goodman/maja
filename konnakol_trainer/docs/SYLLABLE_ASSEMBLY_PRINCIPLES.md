...
Исключение (только для div+xN контекста): при `kalam=fast` и повторяющихся чанках `4`
включается чередование:

- 1-й чанк: `Ta Ka Ju Nu`
- 2-й чанк: `Ta Ka Dhi Mi`
- 3-й: снова `Ta Ka Ju Nu`

Гейт включения:

- у такта есть множитель `rowMultiplier > 1`;
- и это именно div-контекст (`roleType='substitution'` или задан `gatiTargetSub`).

Если множителя нет или это не div-контекст — подмена отключена (дефолтный поток).

Пример: `segLen=12` -> `Dict[4] + Dict[4] + Dict[4]` (а не `9+3`).

## 6.1) Анти-статтер ремонт (повторы)

Scope note: для deterministic legacy/poly пути без random этот этап не применяется.
Текущий no-random словарь/сборка используются как есть, без post-pass ремонта соседних повторов.
...
Scope note: `Thom`-нормализация относится к random/extended token pipeline.
В legacy/poly режиме без random токен `Thom` в базовом deterministic потоке не участвует,
поэтому отдельный no-random этап нормализации `Thom` не выполняется.

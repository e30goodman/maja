 ...
 *
 * Activation contract: arming is explicit in App.tsx with `PressArmSource`:
 * `'star'` (snowflake) vs `'slider'` (Bars thumb) — see `pressMatrixCoordinator.ts`.
...
import type { CellStepMasks } from './stepMask';
...
export type PressBarFrame = {
	...
	stepMasks: Record<number, boolean[]>;
...
export function snapshotBarFrame(r: number, state: PressState): PressBarFrame {
	...
	const stepMasks: Record<number, boolean[]> = {};
	...
	for (const [k, v] of Object.entries(state.cellStepMasks)) {
		const parsed = parseRC(k);
		if (!parsed) continue;
		const [rk, ck] = parsed;
		if (rk !== r) continue;
		if (Array.isArray(v) && v.length > 0) stepMasks[ck] = v.map((x) => Boolean(x));
	}
...
function cloneStateMutable(state: PressState): PressMutable {
	return {
		...
		cellStepMasks: { ...state.cellStepMasks },
...
function clearTargetRow(targetR: number, m: PressMutable, voices: 2 | 3 | 4, isPoly: boolean): void {
	...
	for (const k of Object.keys(m.cellStepMasks)) if (k.startsWith(prefix)) delete m.cellStepMasks[k];
...
function writeFrameInto(
	...
	for (const [cStr, mask] of Object.entries(frame.stepMasks || {})) {
		const c = parseInt(cStr, 10);
		if (!Number.isFinite(c)) continue;
		m.cellStepMasks[`${targetR}-${c}`] = mask.slice();
	}
...
export function dropPress(maxBars: number, state: PressState): PressPatch {
	...
	for (const k of Object.keys(m.cellStepMasks)) {
		const parsed = parseRC(k);
		if (!parsed) continue;
		if (parsed[0] >= maxBars) delete m.cellStepMasks[k];
	}
...
export function isStateEmpty(state: PressState): boolean {
	...
	if (Object.keys(state.cellStepMasks).length > 0) return false;
